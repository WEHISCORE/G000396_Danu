...
# NOTE: This is an ANOVA-like test to identify different (quadratic) time trends
#       in any one of the KOs vs. WT.
tt_quadratic <- topTable(
  fit_tc,
  coef = c("GID1KO.X2", "GID2KO.X2", "GID7KO.X2", "GID8KO.X2", "GID9KO.X2"),
  number = Inf,
  p.value = 0.05)
dim(tt_quadratic)
...
dir.create(here("output", "timecourse"))
tt <- read.csv(
  here("output", "timecourse", "KOs_vs_WT.quadratic.timecourse_DEGs.csv"),
  row.names = 1)

# NOTE: Identify and plot DEGs where the WT has (quadratic) trend with the
#       opposite sign to the KOs.
# NOTE: This uses the average of the pairwise logFCs!
same_sign <- sign(coef(fit_tc)[rownames(tt), "X2"] + tt$logFC) ==
  sign(coef(fit_tc)[rownames(tt), "X2"])
opposite_sign <- !same_sign
# NOTE: This stricter definition checks all pairwise comparisons.
same_sign_strict <-
  sign(coef(fit_tc)[rownames(tt), "X2"] + tt$GID1KO_vs_WT.quadratic.logFC) ==
  sign(coef(fit_tc)[rownames(tt), "X2"]) &
  sign(coef(fit_tc)[rownames(tt), "X2"] + tt$GID2KO_vs_WT.quadratic.logFC) ==
  sign(coef(fit_tc)[rownames(tt), "X2"]) &
  sign(coef(fit_tc)[rownames(tt), "X2"] + tt$GID7KO_vs_WT.quadratic.logFC) ==
  sign(coef(fit_tc)[rownames(tt), "X2"]) &
  sign(coef(fit_tc)[rownames(tt), "X2"] + tt$GID8KO_vs_WT.quadratic.logFC) ==
  sign(coef(fit_tc)[rownames(tt), "X2"]) &
  sign(coef(fit_tc)[rownames(tt), "X2"] + tt$GID9KO_vs_WT.quadratic.logFC) ==
  sign(coef(fit_tc)[rownames(tt), "X2"])
...
# TODO: Could further divide up these plots up based on sign of WT.
pdf(
  here("tmp", "same_sign.quadratic.pdf"),
  width = 9,
  height = 3)
...
pdf(
  here("tmp", "opposite_sign.quadratic.pdf"),
  width = 9,
  height = 3)
# TODO: Remove subsetting.
...
pdf(
  here("tmp", "same_sign_strict.quadratic.pdf"),
  width = 9,
  height = 3)
# TODO: Remove subsetting.
...
pdf(
  here("tmp", "opposite_sign_strict.quadratic.pdf"),
  width = 9,
  height = 3)
# TODO: Remove subsetting.
...
# Using both linear and quadratic trends ---------------------------------------

tt <- topTable(
  fit_tc,
  coef = c(
    "GID1KO.X1", "GID2KO.X1", "GID7KO.X1", "GID8KO.X1", "GID9KO.X1",
    "GID1KO.X2", "GID2KO.X2", "GID7KO.X2", "GID8KO.X2", "GID9KO.X2"),
  number = Inf,
  p.value = 0.05)

same_sign_strict <-
  sign(coef(fit_tc)[rownames(tt), "X1"] + tt$GID1KO.X1) ==
  sign(coef(fit_tc)[rownames(tt), "X1"]) &
  sign(coef(fit_tc)[rownames(tt), "X1"] + tt$GID2KO.X1) ==
  sign(coef(fit_tc)[rownames(tt), "X1"]) &
  sign(coef(fit_tc)[rownames(tt), "X1"] + tt$GID7KO.X1) ==
  sign(coef(fit_tc)[rownames(tt), "X1"]) &
  sign(coef(fit_tc)[rownames(tt), "X1"] + tt$GID8KO.X1) ==
  sign(coef(fit_tc)[rownames(tt), "X1"]) &
  sign(coef(fit_tc)[rownames(tt), "X1"] + tt$GID9KO.X1) ==
  sign(coef(fit_tc)[rownames(tt), "X1"]) &
  sign(coef(fit_tc)[rownames(tt), "X2"] + tt$GID1KO.X2) ==
  sign(coef(fit_tc)[rownames(tt), "X2"]) &
  sign(coef(fit_tc)[rownames(tt), "X2"] + tt$GID2KO.X2) ==
  sign(coef(fit_tc)[rownames(tt), "X2"]) &
  sign(coef(fit_tc)[rownames(tt), "X2"] + tt$GID7KO.X2) ==
  sign(coef(fit_tc)[rownames(tt), "X2"]) &
  sign(coef(fit_tc)[rownames(tt), "X2"] + tt$GID8KO.X2) ==
  sign(coef(fit_tc)[rownames(tt), "X2"]) &
  sign(coef(fit_tc)[rownames(tt), "X2"] + tt$GID9KO.X2) ==
  sign(coef(fit_tc)[rownames(tt), "X2"])
...
pdf(
  here("tmp", "same_sign_strict.both.pdf"),
  width = 9,
  height = 3)
# TODO: Remove subsetting.
for (g in rownames(tt[same_sign_strict & tt$adj.P.Val < 0.05, ])[1:100]) {
...
pdf(
  here("tmp", "opposite_sign_strict.both.pdf"),
  width = 9,
  height = 3)
for (g in rownames(tt[opposite_sign_strict & tt$adj.P.Val < 0.05, ])) {
  message(g)
  x1 <- cbind(data.frame(expression = lcpm[g, ]), y$samples)
  x2 <- cbind(
    data.frame(expression = lcpm_fit_tc[g, ]),
