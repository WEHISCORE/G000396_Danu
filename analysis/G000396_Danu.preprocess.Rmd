...
**TODO** Comment on the data from the empty wells
...
head(sort(reads_ap, decreasing = TRUE))
quantile(reads_ap, 0:10 / 10)
```

```{r}
umis_ap <- DropletUtils::ambientProfileEmpty(
  assay(sce, "UMI_counts"),
  known.empty = which(sce$sample_type == "empty"))
head(sort(umis_ap, decreasing = TRUE))
quantile(umis_ap, 0:10 / 10)
```
...
**TODO** At the technical replicate or biological replicate level?
...
Although Figures \@ref(fig:mds-by-sample-type) - \@ref(fig:aggregated-mds-by-library-size) are only a preliminary summary of the data, there are few patterns worth highlighting:
